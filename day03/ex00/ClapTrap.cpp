...
ClapTrap::ClapTrap(void){std::cout<<"#Default constructor"<<std::endl;}
...
void ClapTrap::attack(std::string const & target){
	if (_energyPoints > 0){
		...
		std::cout<<"ClapTrap "<<_name<<" does not have energy points to inflict damage "<<target<<std::endl;
}
void ClapTrap::takeDamage(unsigned int amount){
	std::cout<<"ClapTrap "<<_name<<" received "<<amount<<" points of damage";
...
void ClapTrap::beRepaired(unsigned int amount){
	std::cout<<"ClapTrap "<<_name<<" restored "<<amount<<" points of energy"<<std::endl;
